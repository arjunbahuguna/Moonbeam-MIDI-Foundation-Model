...
def main(
    ...
    print(
        f"if_add_chords_in_transformer: {if_add_chords_in_transformer}, if_add_metadata_in_transformer: {if_add_metadata_in_transformer}"
    )
    print(
        f"if_add_chords_in_decoder: {if_add_chords_in_decoder}, if_add_metadata_in_decoder: {if_add_metadata_in_decoder}"
    )
    ...
    for i, (_, row) in enumerate(df.iterrows()):
        ...
        if i == min(finished_idx + max_batch_size, len(df) - 1):
            ...
            if if_add_chords_in_transformer:
                chord_token_indices = [
                    [
                        x.index(generator.tokenizer.soc_token_compound),
                        x.index(generator.tokenizer.eoc_token_compound),
                    ]
                    for x in prompts
                ]
            else:
        ...
        chord_midi = create_midi_from_chords(
            chord_condition_str, is_incomplete_measure, ticks_per_bar, normalized_bpm
        )
        ...
        raw_tokens_chord = generator.tokenizer.midi_to_compound(
            chord_save_path, calibate_to_default_tempo=True
        )
        prompts.append(
            generator.tokenizer.encode_series_con_gen_commu(
                None,
                raw_tokens_chord,
