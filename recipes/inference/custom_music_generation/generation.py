...
class MusicLlama:
    ...
    def build(
        ...
        for k, v in checkpoint.items():
            if k.startswith(
                "module."
            ):  # Check if the keys have 'module.' prefix and remove it if necessary
        ...
        if torch.cuda.is_bf16_supported():
            ...
            model = model.to(
                torch.bfloat16
            )  # Explicitly cast the entire model to BF16 precision.
    ...
    def generate(
        ...
        buf_dtype = (
            torch.float32 if getattr(self.config, "microtonal", False) else torch.long
        )
        pad_tensor = (
            torch.tensor(pad_id, dtype=buf_dtype, device="cuda")
            .unsqueeze(0)
            .unsqueeze(0)
        )
        ...
        input_mask = torch.all(tokens != pad_tensor, dim=-1).unsqueeze(
            -1
        )  # (batch, len, 1)
        ...
        for cur_pos in range(
        ...
        ):  # recursively generate new tokens in parallel
            ...
            output = self.model.forward(
                ...
                use_cache=True,
                attention_mask=None,
            )  # output logtis: (batch, len, dim
            next_decoder_token = torch.tensor(
                self.tokenizer.sos_out, dtype=torch.long, device=tokens.device
            ).expand(
                tokens.shape[0] * (cur_pos - prev_pos), 1
            )  # batch*len_x, len_y = 1
            next_decoder_token_out = next_decoder_token
            hidden_state = (
                output.logits
            )  # first forward pass: batch, len_x, dim --> batch*len_x, dim  --> num_layer, batch*len_x, dim;
            hidden_state = (
                hidden_state.view(
                    hidden_state.shape[0] * hidden_state.shape[1], hidden_state.shape[2]
                )
                .unsqueeze(0)
                .expand(self.model.decoder.num_hidden_layers, -1, -1)
                .contiguous()
            )  # batch, len_x, dim --> num_layer, batch*len_x, dim;

            for attribute in [
                "timeshift_dict_decode",
                "duration_dict_decode",
                "octave_dict_decode",
                "pitch_dict_decode",
                "instrument_dict_decode",
                "velocity_dict_decode",
            ]:
                output_decoder = self.model.forward(
                    decoded_hidden_state=hidden_state,
                    decoded_language_tokens=next_decoder_token,
                    attention_mask=None,
                )
                generation_logits = (
                    output_decoder.generation_logits
                )  ##batch*len_x, len_y, decode_vocab_size
                hidden_state = (
                    output_decoder.generation_hidden_state
                ...
                if temperature > 0:
                    probs = torch.softmax(
                        generation_logits[:, -1, :] / temperature, dim=-1
                    )
                    next_decoder_token = sample_top_p(probs, top_p)

                    for i in range(
                        next_decoder_token.size(0)
                    ):  # Ensure that all next_decoder_token values are in sample_indices, print warning if probability mass of all allowed indices is smaller than 0.8
                        start_time = time.time()
                        while (
                            next_decoder_token[i, 0].item() not in sample_indices_set
                        ):  # Check if token is valid
                            if (
                                time.time() - start_time > 15
                            ):  # If sampling takes too long, mask invalid indices
                                print(
                                    f"Warning: Resampling for token {i} exceeded 15 seconds. Masking invalid logits and Resampling..."
                                )
                    ...
                    probs = torch.softmax(
                        generation_logits[:, -1, :], dim=-1
                    )  # batch*len_x, len_y (last), decode_vocab_size
                    sample_indices_tensor = torch.tensor(
                        sample_indices, device=probs.device
                    )  # Ensure it's on the same device as probs
                    probs_at_sample_indices = probs[
                        :, sample_indices_tensor
                    ]  # Shape: [batch_size, num_sample_indices]
                    next_token_index_in_subset = probs_at_sample_indices.argmax(
                        dim=-1, keepdim=True
                    )  # Shape: [batch_size, 1]
                    next_decoder_token = sample_indices_tensor[
                        next_token_index_in_subset.squeeze(-1)
                    ].unsqueeze(
                        -1
                    )  # Shape: [batch_size]
                ...
                probs_at_sample_indices = probs[
                    :, sample_indices
                ]  # Extract probabilities for the sampled indices
                cumulative_prob = probs_at_sample_indices.sum(
                    dim=-1
                )  # Sum over the sampled indices to get cumulative prob
                num_samples_below_threshold = (
                    (cumulative_prob < 0.8).sum().item()
                )  # Count the number of True values
                if num_samples_below_threshold > 0:
                    print(
                        f"{num_samples_below_threshold} / {cumulative_prob.shape[0]} samples have a cumulative probability < 0.8 at the allowed indices"
                    )

                next_decoder_token_out = torch.cat(
                    [next_decoder_token_out, next_decoder_token], dim=-1
                )  # batch*len_x, len_y

            # remove the sos_out token
            next_decoder_token_out_reshaped = next_decoder_token_out[:, 1:].view(
                tokens.shape[0], -1, 6
            )  # batch*len_x, 6 --> batch, len_x, 6
            next_decoder_token_lang = self.tokenizer.convert_from_language_tokens(
                next_decoder_token_out_reshaped
            )  # batch, lenx, 6

            previous_onset = tokens[:, cur_pos - 1, 0]  # batch,
            new_onset = previous_onset + next_decoder_token_lang.clone().detach()[
                :, -1, 0
            ].to(
                previous_onset
            )  # batch, + batch --> batch
            next_decoder_token_onset = torch.cat(
                [
                    new_onset.unsqueeze(-1),
                    next_decoder_token_lang.clone().detach()[:, -1, 1:],
                ],
                dim=-1,
            ).to(
                tokens
            ...
            eos_conditions_onset = (
                next_decoder_token_lang.clone().detach()[:, -1, 0]
            ...
            eos_conditions_oct = (
                next_decoder_token_lang.clone().detach()[:, -1, 2]
                == self.tokenizer.eos_octave
            )  # batch,
            eos_conditions_pitch = (
                next_decoder_token_lang.clone().detach()[:, -1, 3]
                == self.tokenizer.eos_pitch_class
            )  # batch,
            eos_conditions_instr = (
                next_decoder_token_lang.clone().detach()[:, -1, 4]
                == self.tokenizer.eos_instrument
            )  # batch,
            eos_conditions_vel = (
                next_decoder_token_lang.clone().detach()[:, -1, 5]
            ...
            eos_conditions = torch.any(eos_conditions_all_attr, dim=-1).to(
                input_mask
            )  # batch, 1
        ...
        return [
            {
                "generation": {
                    ...
                    "content": self.tokenizer.compound_to_midi(
                        t,
                        microtonal=self.tokenizer.microtonal,
