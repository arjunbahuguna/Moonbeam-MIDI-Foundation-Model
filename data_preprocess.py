...
try:
    ...
    try:
        MusicTokenizer = importlib.import_module(
            "llama_recipes.datasets.music_tokenizer"
        ).MusicTokenizer
    except Exception:
        raise RuntimeError(
...
def detect_large_timeshifts_and_durations(compounds, onset_vocab_size, dur_vocab_size):
    ...
    )  # why -3? onset_vocab_size -1, onset_vocab_size -2 are assigned to sos and eos, max value is onset_vocab_size - 3
    duration_exceed_vocab_size = any(
        key > dur_vocab_size - 3 for key in duration_counter.keys()
    )  # if dur_vocab_size=1026, 1024 and 1025 are reserved for sos and eos, the largest value becomes 1023
    return (
...
def filter_large_ts_dur(
    ...
    if onsets_exceed_vocab_size or duration_exceed_vocab_size:
        with open(log_file, "a") as log:
            log.write(
                f"Failed to process {output_file_path} but during finetuning these files will be kept:\n"
            )
            log.write(
...
def process_midi_file_v3(
    ...
    output_file_name = (
        midi_file.replace("/", "_").replace(".midi", ".npy").replace(".mid", ".npy")
    )
        ...
        if len(list_of_compounds) == 1:
            ...
            list_of_output_file_path = [
                os.path.join(
                    output_folder,
                    midi_file.split("/")[-1]
                    .replace(".midi", f"_{i}.npy")
                    .replace(".mid", f"_{i}.npy"),
                )
            ...
            return [
                ...
                for (compounds, output_file_path) in zip(
                    list_of_compounds, list_of_output_file_path
                )
            ]
    else:
        return [
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Process MIDI files for the MIDI dataset."
    )
    ...
    parser.add_argument(
        "--dataset_folder",
        type=str,
        help="Path to the dataset folder containing MIDI files.",
    )
    parser.add_argument(
        "--output_folder",
        type=str,
        help="Path to the folder where processed files will be saved.",
    )
    parser.add_argument(
        "--model_config",
        type=str,
        help="Model configuration file that decides the vocab size",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="If Timeshift exceeds this value, chunk the file",
    )
    ...
    if not args.train_test_split_file:
        train_files, test_files = train_test_split(
            find_midi_files(args.dataset_folder),
        ...
        midi_files, splits, labels = find_midi_files_from_file(
    ...
    with open(args.model_config, "r") as file:
        data = json.load(file)
        onset_vocab_size = data.get(
            "onset_vocab_size", None
        )  # value - 2 = max value of timeshift
        dur_vocab_size = data.get(
            "dur_vocab_size", None
        )  # value - 2 = max value of duration
    ...
    ]  # results contains of list of lists, flatten it

    csv_rows = []
    results_success = [result for result in results if result is not None]
    for result in results_success:
        csv_rows.append(
            [os.path.basename(result["file"]), result["split"], result["label"]]
        )
    ...
    print(
        f"Processed {len(midi_files)} files including {len(results)} chunks with {len(results)-len(results_success)} failures. CSV file saved to {csv_file_path}"
    )
