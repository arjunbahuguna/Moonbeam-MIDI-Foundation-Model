...
def generate_dataset_config(train_config, kwargs):
    ...
    dataset_cls = getattr(datasets, train_config.dataset, None)

    if dataset_cls is None:
        # Fallback for player classification when configs.datasets is stale/inconsistent.
        if train_config.dataset == "player_classification_dataset":
            ...
            dataset_config = SimpleNamespace(
                ...
                train_split="train",
                test_split="test",
                data_dir="data/processed_datasets/classification/gpm30",
                csv_file="data/processed_datasets/classification/gpm30/train_test_split.csv",
                seq_len=1200,
                seq_dur=None,
                non_overlap_ratio=0.25,
                individual_eval=True,
            )
            update_config(dataset_config, **kwargs)
            return dataset_config

        available = [n for n, v in inspect.getmembers(datasets, inspect.isclass)]
        raise KeyError(
            f"Dataset config class '{train_config.dataset}' not found in configs.datasets. "
