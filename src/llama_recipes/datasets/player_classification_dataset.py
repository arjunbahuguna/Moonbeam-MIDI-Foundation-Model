...
def process_file(args):
    ...
    while fast < len(raw_tokens):
        ...
        if current_duration <= seq_len:
            ...
            raw_tokens_adjust_onset = np.array(
                [
                    [event[0] - abs_onset] + event[1:]
                    for event in raw_tokens[slow:fast].tolist()
                ]
            )
            ...
            if partition == "train":  # Create overlap
                ...
                if slow_new == slow:  # Ensure progress
                    print(
                        f"breaking the loop to avoid infinite loop: {slow_new} slow old{slow}, len(raw_tokens): {len(raw_tokens)}"
                    )
                    slow += 1
                    # break
                else:
...
class PlayerClassificationDataset(Dataset):
    def __init__(self, dataset_config, tokenizer, partition="train"):
        ...
        self.file_basenames = [
            f for f, s in zip(file_basenames, splits) if s == partition
        ]
        ...
        if dataset_config.individual_eval and partition == "test":
            ...
            self.chunked_files, self.chunked_labels = [
                np.load(os.path.join(self.data_dir, "processed", filename))
                for filename in self.file_basenames
            ], self.labels
        else:
            if self.seq_len and not self.seq_dur:  # chunk file based on seq_len
                print(f"Chunk file based on seq_len = {self.seq_len} events")
                self.chunked_files, self.chunked_labels = (
                    self.get_chunked_files_labels_based_on_seq_len()
                )
                ...
                self.chunked_files, self.chunked_labels = (
                    self.get_chunked_files_labels_based_on_dur_parallel()
                )
            elif (
                not self.seq_dur and not self.seq_len
            ):  # concat all events in midi file
    ...
    def __getitem__(self, index):
        ...
        encoded_tokens = self.tokenizer.encode_series_player_classification(
            self.chunked_files[index], if_add_sos=True, if_add_eos=True
        )  # SOS and EOS are added in the tokenizer

        classification_labels = [
            self.chunked_labels[index] for _ in range(len(encoded_tokens))
        ]
        ...
        return {
            ...
            "attention_mask": [
                0 for _ in range(len(encoded_tokens))
