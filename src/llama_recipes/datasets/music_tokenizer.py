...
class MusicTokenizer:
    def __init__(
        ...
        self.sos_timeshift, self.eos_timeshift = (
            self.timeshift_vocab_size - 2,
            self.timeshift_vocab_size - 1,
        )  # TODO think whether this is correct
        ...
        self.sos_octave, self.eos_octave = (
            self.octave_vocab_size - 2,
            self.octave_vocab_size - 1,
        )
        if self.microtonal:
            ...
            self.sos_pitch_class, self.eos_pitch_class = (
                self.pitch_class_vocab_size - 2,
                self.pitch_class_vocab_size - 1,
            )
        self.sos_instrument, self.eos_instrument = (
            self.instrument_vocab_size - 2,
            self.instrument_vocab_size - 1,
        )
        self.sos_velocity, self.eos_velocity = (
            self.velocity_vocab_size - 2,
            self.velocity_vocab_size - 1,
        )
        ...
        self.timeshift_dict = {
            i: i + self.sos_out_vocab_size for i in range(self.timeshift_vocab_size)
        }
        self.duration_dict = {
            i: i + self.sos_out_vocab_size + self.timeshift_vocab_size
            for i in range(self.dur_vocab_size)
        }  # linear scale
        self.octave_dict = {
            i: i
            + self.sos_out_vocab_size
            + self.timeshift_vocab_size
            + self.dur_vocab_size
            for i in range(self.octave_vocab_size)
        }

        # Offsets derived from config, used by training scripts and model code
        self._original_pitch_count = (
            14  # 12 chromatic pitches + SOS + EOS (pretrained constant)
        )
        self.pitch_offset = (
            self.sos_out_vocab_size
            + self.timeshift_vocab_size
        ...
        if self.microtonal:
            ...
            for idx in range(num_pitches):
                c = (
                    idx * resolution
                )  # actual cent value (e.g., 0,1,2,...,1199 or 0,10,20,...,1190)
                if c % 100 == 0:
            ...
            self.instrument_dict = {
                i: instr_offset + i for i in range(self.instrument_vocab_size)
            }
            vel_offset = instr_offset + self.instrument_vocab_size
            self.velocity_dict = {
                i: vel_offset + i for i in range(self.velocity_vocab_size)
            }
        else:
            self.pitch_dict = {
                i: i
        ...
        print(
            f"self.sos_out_dict:{self.sos_out_dict}, self.timeshift_dict:{self.timeshift_dict}, self.duration_dict:{self.duration_dict},self.octave_dict:{self.octave_dict}, self.pitch_dict (first 5):{dict(list(self.pitch_dict.items())[:5])}, self.instrument_dict (first 3):{dict(list(self.instrument_dict.items())[:3])} self.velocity_dict (first 3):{dict(list(self.velocity_dict.items())[:3])}"
        )
    ...
    def encode_series_labels(self, encoded_tokens, if_added_sos, if_added_eos):
        encoded_tokens = torch.tensor(
            encoded_tokens
        )  # temporarily convert to tensor for easier slicing (len, 6)
        ...
        timeshift_labels_raw = torch.diff(
            encoded_tokens[:, 0], prepend=torch.tensor([0])
        )
        ...
        output = torch.cat(
            [
                torch.zeros(encoded_tokens.shape[0]).unsqueeze(-1),
                timeshift_labels_raw.unsqueeze(-1),
                encoded_tokens[:, 1:],
            ],
            dim=-1,
        )  # (len, 6) #TODO: check if this is correct, ensure there is no time shift < 0
    ...
    def encode_series_con_gen_emotion(
        self, raw_token_series, if_add_sos, if_add_eos, emotion_token_4Q=None
    ):
    ...
    def encode_series_labels_con_gen_emotion(
        self, encoded_tokens, if_added_sos, if_added_eos, if_added_emotion_token_4Q=None
    ):
        ...
        if (
            if_added_emotion_token_4Q is not None
        ):  # emotion token should always followed by the sos token in this task, we can use dummy labels (self.sos_label) for emotion_token_4Q tokens.
            output = torch.concat(
                [torch.tensor(self.sos_label).unsqueeze(0), output], dim=0
            )
    ...
    def encode_series_con_gen_commu(
        self,
        raw_token_series,
        raw_chord_series,
        metadata_tokens,
        if_only_keep_condition_tokens=False,
        if_add_chords_in_transformer=True,
        if_add_metadata_in_transformer=False,
    ):
        ...
        if if_add_chords_in_transformer:
            ...
            out_chord = (
                [self.soc_token_compound] + out_chord + [self.eoc_token_compound]
            )
        else:
            out_chord = []
    ...
    def encode_series_labels_con_gen_commu(self, encoded_tokens):
        ...
        sos_idx = (
            (encoded_tokens == torch.tensor(self.sos_token_compound))
            .all(dim=1)
            .nonzero(as_tuple=True)[0]
        )
        eos_idx = (
            (encoded_tokens == torch.tensor(self.eos_token_compound))
            .all(dim=1)
            .nonzero(as_tuple=True)[0]
        )
        ...
        output = [
            self.sos_label for _ in range(sos_idx[0])
        ] + output  # since we are not really predicting the chord and metadata tokens, we can use dummy labels (self.sos_label) for these tokens

        # 4. encode it to labels
        labels = [self.convert_to_language_tokens(x) for x in output]
        return labels

    def encode_series_player_classification(
        self, raw_token_series, if_add_sos, if_add_eos, player_label=None
    ):
        """
        Encode a series of tokens with player classification label.

        Args:
            raw_token_series: list of raw tokens
            if_add_sos: whether to add start-of-sequence token
            if_add_eos: whether to add end-of-sequence token
            player_label: player classification label (or makam)

        Returns:
            Encoded token series with player label prepended
        """
        out = [self.encode_single(x) for x in raw_token_series]
        if if_add_sos:
            out = [self.sos_token_compound] + out
        ...
        if player_label is not None:
            # Prepend player label as a compound token (replicate across 6 dimensions)
            out = [[player_label for _ in range(6)]] + out
    ...
    def create_dur_dictionary(self):
        ...
        bin_width = (
            10 / dur_vocab_size_wo_soseos
        )  # assume duration ranges from 10ms to 10240ms
        # Generate bin boundaries
        bin_boundaries = [
            2 ** (i * bin_width) for i in range(dur_vocab_size_wo_soseos + 1)
        ]
        ...
        for token in range(1, 1025):  # Tokens from 1 to 1024
            ...
            for bin_num in range(dur_vocab_size_wo_soseos):
                if bin_boundaries[bin_num] <= token < bin_boundaries[bin_num + 1]:
                    token_to_bin[token] = (
                        bin_num + self.timeshift_vocab_size + self.sos_out_vocab_size
                    )
        ...
        token_to_bin[self.sos_dur] = (
            self.dur_vocab_size
            - 2
            + self.timeshift_vocab_size
            + self.sos_out_vocab_size
        )  # VERY UGLY FIX
        token_to_bin[self.eos_dur] = (
            self.dur_vocab_size
            - 1
            + self.timeshift_vocab_size
            + self.sos_out_vocab_size
    ...
    def binary_to_decimal_batch(binary):
        ...
        bits = binary.size(
            -1
        )  # 2 bits are reserved for sos and eos TODO: add exception handling script, e.g., no eos and sos token is added, return dtype?
    ...
    def midi_to_compound(
        ...
        TIME_RESOLUTION=100,
        debug=False,
        calibate_to_default_tempo=False,
    ):
        """
        ...
        tokens = (
            []
        )  # output contains tuples of (onset, duration, octave, pitch_class, instr, velocity)
        note_idx = 0
        ...
        for message in midi:
            ...
            if message.type == "program_change":
                instruments[message.channel] = (
                    message.program
                )  # assign an instrument name to that channel
                ...
                if message.type == "note_on" and message.velocity > 0:  # onset
                    ...
                    if self.microtonal:
                        ...
                        canonical_pitch = (
                            message.note + bend_semitones
                        )  # MIDI note is integer (60 for C4), bend_semitones is a float (-0.5 semitone for 2048 pitchbend value) ===> canonical_pitch is a float semitone value like 59.5
                        octave, pitch_class = pitch_to_octave_pitch_class_microtonal(
                            canonical_pitch
                        )
                        # Round to nearest resolution step
                        if self.microtonal_resolution > 1:
                            pitch_class = (
                                round(pitch_class / self.microtonal_resolution)
                                * self.microtonal_resolution
                            )
                    ...
                    try:
                        ...
                        if calibate_to_default_tempo:
                            duration_ticks = round(
                                TIME_RESOLUTION * 500000 / tempo * (time - onset_time)
                            )
                        else:
                            duration_ticks = round(
                                TIME_RESOLUTION * (time - onset_time)
                            )
                        if duration_ticks == 0:
                            duration_ticks = 1
        ...
        if filtered_note0:
            ...
            if debug:
                print(
                    f"NOTE-0 FILTER: {len(filtered_note0)} rest marker(s) removed from {fname}"
                )
                for entry in filtered_note0:
                    print(
                        f'  time={entry["time"]:.4f}s (tick={entry["time_ticks"]}), '
    ...
    def compound_to_midi(
        ...
        for _, (row) in enumerate(tokens):
            ...
            time_index[(time_in_ticks, 0)].append(
                (note, instrument, velocity, pitchbend)
            )
            time_index[(time_in_ticks + duration, 1)].append(
                (note, instrument, velocity, 0)
            )

        track_idx = {}  # maps instrument to (track, current_time, channel_idx)
        num_tracks = 0
        for time_in_ticks, event_type in sorted(time_index.keys()):
            for note, instrument, velocity, pb in time_index[
                (time_in_ticks, event_type)
            ]:
                    ...
                    try:
                        ...
                        if instrument == 128:  # drums always go on channel 9
                            ...
                            message = mido.Message(
                                "program_change", channel=idx, program=0
                            )
                        else:
                            message = mido.Message(
