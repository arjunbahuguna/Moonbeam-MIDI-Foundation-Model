...
def setup_wandb(train_config, fsdp_config, llama_config, **kwargs):
    ...
    init_dict["name"] = "only_symbtr_WE" # Optional: set a custom run name for easier identification in the dashboard]
...
def main(**kwargs):
    ...
    if train_config.enable_fsdp and train_config.low_cpu_fsdp:
        ...
        if train_config.train_from_scratch:
            print("=" * 60)
            print("TRAINING FROM SCRATCH: Skipping checkpoint loading and vocab expansion")
            print("=" * 60)
        else:
    ...
    # NOTE: Regularization is DISABLED when training from scratch (no pretrained weights to preserve)
    microtonal_reg = None
    if getattr(llama_config, 'microtonal', False) and not train_config.train_from_scratch:
    ...
    # NOTE: Disabled when training from scratch (no pretrained western rows to protect)
    if microtonal_reg is not None and not train_config.train_from_scratch:
    ...
    # NOTE: Gradient scaling hook is only used when NOT training from scratch
    if getattr(llama_config, 'microtonal', False) and not train_config.train_from_scratch:
