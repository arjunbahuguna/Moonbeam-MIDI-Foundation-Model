...
def run_poststats(input_dir):
    ...
    for fpath in orig_files:
        notes, _ = parse_notes_from_midi(fpath)
        orig_canonical.extend(n['canonical_pitch'] for n in notes)

    orig_pc_cents = [int(round((p % 12) * 100)) for p in orig_canonical]
    orig_unique_bins = len(set(orig_pc_cents))

    # Augmented stats
    aug_canonical = []
    for fpath in aug_files:
        notes, _ = parse_notes_from_midi(fpath)
        aug_canonical.extend(n['canonical_pitch'] for n in notes)

    aug_pc_cents = [int(round((p % 12) * 100)) for p in aug_canonical]
    aug_unique_bins = len(set(aug_pc_cents))

    print(f"  {'Metric':<35} {'Original':>15} {'Augmented':>15}")
    print(f"  {'─'*65}")
    print(f"  {'Total files':<35} {len(orig_files):>15} {len(aug_files):>15}")
    print(f"  {'Total notes':<35} {len(orig_canonical):>15} {len(aug_canonical):>15}")
    print(f"  {'Unique cent bins (mod 1200)':<35} {orig_unique_bins:>15} {aug_unique_bins:>15}")
    if orig_canonical:
        print(f"  {'Pitch range (cents)':<35} {min(orig_canonical)*100:>12.1f}–{max(orig_canonical)*100:.1f}")
    if aug_canonical:
        print(f"  {'Pitch range augmented (cents)':<35} {min(aug_canonical)*100:>12.1f}–{max(aug_canonical)*100:.1f}")

    # ── Side-by-side histograms ──
    fig, axes = plt.subplots(2, 1, figsize=(14, 8))

    axes[0].hist(orig_pc_cents, bins=range(0, 1201, 1), edgecolor='none', alpha=0.8, color='steelblue')
    axes[0].set_xlabel('Pitch Class (cents, 0–1199)')
    axes[0].set_ylabel('Count')
    axes[0].set_title(f'Original — {len(orig_files)} files, {len(orig_canonical)} notes, {orig_unique_bins} unique bins')

    axes[1].hist(aug_pc_cents, bins=range(0, 1201, 1), edgecolor='none', alpha=0.8, color='coral')
    axes[1].set_xlabel('Pitch Class (cents, 0–1199)')
    axes[1].set_ylabel('Count')
    axes[1].set_title(f'Augmented — {len(aug_files)} files, {len(aug_canonical)} notes, {aug_unique_bins} unique bins')
    ...
    aug_cents_full = [p * 100 for p in aug_canonical]
    fig, ax = plt.subplots(figsize=(14, 4))
    ax.hist(aug_cents_full, bins=range(0, 12800, 10), edgecolor='none', alpha=0.8, color='coral')
    ax.set_xlabel('Canonical Pitch (cents)')
    ax.set_ylabel('Count')
    ax.set_title(f'Canonical Pitch Distribution — Augmented ({len(aug_files)} files)')
    plt.tight_layout()
    ...
    # Save phase 5 stats
    phase5_stats = {
        'original_files': len(orig_files),
        'augmented_files': len(aug_files),
        'original_notes': len(orig_canonical),
        'augmented_notes': len(aug_canonical),
